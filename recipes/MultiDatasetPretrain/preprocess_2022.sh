...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for subset in "train" "validation"; do
        list_path="${list_dir}/${subset}_2022.txt"

        python ./local/save_2022_list.py \
        --config-dir "./conf" \
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for subset in "train" "validation"; do
        subset_list_path="${list_dir}/${subset}_2022.txt"
        subset_feature_dir="${feature_dir}/${subset}_2022"
